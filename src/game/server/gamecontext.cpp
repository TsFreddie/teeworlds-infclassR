...
{
	//Simple nerf without a formula yet (TODO)
	m_NerfFactor = 100;
	...
	bool refresh = true;
	InitializeWeaponParams(refresh);			//standard weapons need a refresh
	SendChatTarget_Localization(-1, CHATCATEGORY_PLAYER, _("Player info: Low player count - weapon damage reduction active"));
	dbg_msg("game", "Low player count - weapon damage reduction active");
}
...
void CGameContext::SaveWeaponConfig() {
	...
	if (configVector[12] == 0) //m_InfMercGunAmmoRegenTime is never null, if it is vector is faulty
	{
		dbg_msg("game", "fatal error in GameContext::SaveWeaponConfig() - WeaponConfig not saved");
		return;
	}
...
void CGameContext::ResetWeaponConfig(std::vector<int> value) {
	...
	if (value.size() > 0) 
	{
		...
		SendChatTarget_Localization(-1, CHATCATEGORY_PLAYER, _("Player info: High player count - weapon damage reduction inactive"));
		dbg_msg("game", "High player count - weapon damage reduction inactive");
	}
	else
	{
		dbg_msg("game", "fatal error in GameContext::ResetWeaponConfig() - unable to undo NerfWeapons()");
	}
